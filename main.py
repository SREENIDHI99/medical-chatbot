...
def medication(sentence):
    for word in sentence.split():
        ...
        if pp in MEDICATION:
            soln=MEDICATION[pp]
            print("so the suggested tablets for your problem are")
            return soln
        ...
        if (userText != 'bye'):
            if (userText == 'thanks' or userText == 'thank you'):
                ...
                if (greeting(userText) != None):
                    return("ROBO: " + greeting(userText))
                elif (first(userText) != None):
                    return(first(userText))
                elif (medication(userText) != None):
                    return(medication(userText))
